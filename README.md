...
├── workflows/                agent skills that automate the pipeline (see below)
...
**To run them:** open this repo with an AI coding assistant that supports the Agent Skills format, point it at `workflows/`, then ask it to "run the experiment analysis" or "check if this new dataset is well-powered" — it will find and invoke the right skill automatically. No separate install step; the skill definitions just wrap the same CLI commands shown above.
